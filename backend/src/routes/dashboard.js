...
function createDashboardRouter(pool) {
  ...
  router.get('/ward/:ward_id', async (req, res) => {
    try {
      ...
      const totalQuery = `SELECT COUNT(*) AS count FROM incidents WHERE ward_id = $1`;
      const totalResult = await pool.query(totalQuery, [ward_id]);
      const totalIncidents = parseInt(totalResult.rows[0].count, 10) || 0;

      const resolvedQuery = `SELECT COUNT(*) AS count FROM incidents WHERE ward_id = $1 AND status = 'resolved'`;
      const resolvedResult = await pool.query(resolvedQuery, [ward_id]);
      const resolvedCount = parseInt(resolvedResult.rows[0].count, 10) || 0;

      const openQuery = `SELECT COUNT(*) AS count FROM incidents WHERE ward_id = $1 AND status != 'resolved'`;
      ...
      const avgResponseQuery = `
        SELECT AVG(EXTRACT(EPOCH FROM (resolved_at - first_reported_at)) / 3600) AS avg_hours
        FROM incidents
        WHERE ward_id = $1 AND status = 'resolved' AND resolved_at IS NOT NULL
      `;
      ...
      `;
      const byStatusResult = await pool.query(byStatusQuery, [ward_id]);

      const sixtyDaysFromNow = new Date();
      sixtyDaysFromNow.setDate(sixtyDaysFromNow.getDate() - 60);

      const pendingQuery = `
        SELECT id, issue_type, severity, landmark_description,
               report_count, first_reported_at
        FROM incidents
        WHERE ward_id = $1 AND status = 'reported'
          AND first_reported_at >= $2
        ORDER BY first_reported_at DESC
      `;
      const pendingResult = await pool.query(pendingQuery, [ward_id, sixtyDaysFromNow.toISOString()]);
